 ...
 *
 * ⚠️ Por qué no usamos `loadEnv` de Vite ni `process.loadEnvFile()`: los dos incluyen
 * `process.env`, y esas variables PISAN a las del archivo. Si el sistema del usuario tiene un
 * MONDAY_TOKEN viejo (de otro cliente, por ejemplo), ese gana: editás el .env.local, no cambia
 * nada, y te volvés loco buscando el problema en otro lado. Es un error real que ya pasó dos
 * veces. Acá el archivo manda, que es lo que uno espera al editarlo.
 */
 ...
 *
 * Antes había que usar `vercel dev`, lo que obligaba a instalar la CLI y crearse una cuenta solo
 * para ver la app con datos reales. Con esto alcanza `npm run dev`.
 *
 * Reusa EL MISMO `api/monday.js` que se despliega en producción — no hay una segunda
 * implementación que se pueda desincronizar, y de paso los filtros de seguridad del proxy se
 * prueban también en local. Solo traduce entre el middleware de Vite (req/res crudos de Node) y
 * la firma que espera una función de Vercel.
 *
